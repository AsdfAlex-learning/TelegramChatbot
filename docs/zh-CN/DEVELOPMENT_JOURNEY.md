...
又是一位神秘的大佬说：“你也不希望你的数字伴侣被别人滥用吧？”

确实，无论是这个项目还是其他的项目开发过程中，关于权限管理的问题也一直是非常重要的一个问题。最初的想法是做一个完全公开的 Bot，但考虑到 API 成本和个人隐私（尤其是 Prompt 设定），需要引入权限控制

根据AI的建议，最初我考虑过复杂的 RBAC（基于角色的访问控制），或者使用数据库存储管理员列表。但仔细一想，对于这样一个个人性质较强、甚至说是几乎极端的私有化的项目，过于复杂的权限系统反而增加了维护成本。最终确定了一个极简的方案：**Owner-Only**，只有项目的部署者才能够进行权限设置

在 `config.yaml` 中直接配置 `owner_id`。这个 ID 拥有最高权限，且无法通过指令修改，必须修改配置文件。这保证了基本上绝对的安全性

同时我引入一个全局变量 `bot_is_private`，作为私有化的开关。开启时 只有 Owner 可以对话，其他用户的请求会被拦截或忽略，而 关闭时 所有人都可以对话（公开模式）

Owner 可以通过 `/set_private true` 或 `/set_private false` 随时切换模式。例如，调试时开启私有模式，稳定后开放给朋友使用

这么做 简单 安全 还比较灵活。不需要数据库表来存管理员（也就避免了数据库注入的可能性），不需要复杂的鉴权逻辑。Owner ID 写死在文件里，不会因为数据库被黑或者逻辑漏洞导致权限旁落，还能通过指令动态切换私有/公开，既能保护隐私，又能随时分享

总之关于这部分的设置我还是觉得蛮不错的，毕竟这并不是什么非常大的项目，每个人都有自己想说的悄悄话，也有自己的隐私，不希望被人看到

# 随着项目的变大，代码也需要一部分的“断舍离”，要从“野路子”到“有组织的正规军”

随着功能的增加，我在`main.py`内慢慢堆积了非常多的代码，其中负责各个功能的代码都混在了同一个文件之中，导致今后如果不使用`telebot`或者是`nonebot`这两种架构的话，我需要花费非常多的时间重构绝大部分代码。不仅如此，项目结构非常缺乏可维护性，不方便别人维护/重构，完全不能当作一个“标准后端项目”

我在AI的建议下开始拆分`main.py`，同时也在规范关于原本`system_config.json`这种设计。我创建了`SessionController`、`ChatService`、`InteractionManager` 等核心组件，慢慢的让整个项目变得更符合一个后端的项目了。

唉 虽然确实越来越像一个标准的后端，但是我还是希望我喜欢的人能够看到我的变化，希望得到他的肯定与鼓励

不过和他之间的差距还是太大 太大......真正开始接触这些开发之后我才感受到我和他之间的差距，我差的太多太多了

说回`main.py`吧。我把一切的起点都放在了`main.py`：先创建唯一的`SessionController`（配置的唯一真理来源），把它传给`ChatService`，再把 `ChatService`和`SessionController`传给`InteractionManager`，然后只需要一个`SessionController`的实例就可以控制机器人的权限，同时不会因为创建多个实例而导致权限混乱

这样，数据流向非常清晰：`main -> Controller -> Service -> Manager`

然后是关于`system_config.json`的设计。这样的设计虽然结构很简单，但是并不能真正的实现数据化 结构化的配置。我希望未来可以考虑使用数据库来存储这些配置，而不是依赖于一个静态的json文件。同时，关于prompt的设计也需要考虑一下。当前的设计是将prompt直接写在json文件中，这在一定程度上是方便了开发，但是也暴露了一些问题。例如，prompt的内容如果需要修改，就需要修改json文件，这会导致整个项目的代码都需要重新编译。同时，prompt的内容如果需要根据用户的输入进行动态调整，就需要在代码中进行复杂的逻辑处理。这些问题都需要在未来的设计中考虑到（因为我正在酝酿一个更大的体系，后续会慢慢更新到，在文档内先卖个关子）

这次重构让代码结构更清晰了，虽然功能没变，好像也确实更符合规范了，但是我反而更心慌了：我的思维没能跟上AI给我的建议，慢慢的开始有一种控制正在丧失的感觉：我已经不确定我现在写的东西，是不是我自己真正理解的。我需要静下心真正的开始去学习

# 我真的需要这些东西吗？我似乎从来没有依赖过，我只是在使用一个让我觉得有安全感的框架

在不断的重构中，AI进行code review后，提示我 我并没有使用任何的`NoneBot`的功能。也就是说，我自以为基于`NoneBot`的机器人，其实最终几乎完全不构成任何一点的依赖。我之前只是尝试使用一个让我觉得有`安全感`的框架

我做了一个有些大胆但是很应该做的决定：完全移除`NoneBot`框架。虽然`NoneBot`是一个优秀的框架，但对于我目前完成的这部分代码来说，我似乎确实不需要那么多的功能，同时在`NoneBot`自动安装的时候也因为某一些问题，基础的echo功能都无法正常工作，只能放弃使用`NoneBot`------早在最基础的阶段就已经没在使用`NoneBot`了，这个时候删除`NoneBot`也不会对我目前的代码产生任何影响

就像是我的生活一样，其实他从来没有教会我怎么写代码，他只是引导我的兴趣，让我从最开始的简单的echo功能开始，一步一步的去自己探索。但他从来都不是我生活的依赖，只是我以为我需要他，所以我才会在我的`requirements.txt`中引入他

说回项目架构吧。现在的架构更加清晰：启动逻辑简化为一个 `while True` 循环，不再需要`NoneBot`的相关内容。同时，以前的`main.py`完全可以被称为`God file`，在经过重构后职责也变得非常清晰：仅负责启动和顶层异常捕获。`src/bot/wiring.py`专门负责依赖注入和对象组装（避免了 God Object），而`src/bot/app.py`则负责核心业务编排，完全不知道 Telegram 的存在（这也保证了 如果需要更换平台，这部分核心内容不需要进行多少的变动）`src/bot/telegram/`专门处理 Telegram 协议细节（Client, Polling, Handlers），作为 IO 适配层

# 目前的架构大抵是足够清晰了，我可能需要开始涉足LLM的相关内容了

在做这个项目之前，我只是一个非常普通，甚至说完全没有什么开发经验的人。我曾经只是一个混吃等死的学生，从来没有过任何的开发经验，大概率毕业后就会失业，走上一条可能很多人都会走上的道路。我从来没想过关于如何写一个符合后端标准的项目，没听过`MVC`，没听过`DDD`，没听过`Clean Architecture`，也没听过`God file`，不理解为什么需要拆分已经写好了的.py文件，等等

我在这个项目之前从来没想过我能走到现在，从来没有想要往前走的更远。但既然我已经开始了，我就必须要走下去，还得走的更远，至少也要走到能够和他共事的程度

很感谢我从未正式拜在他门下但给予我非常多帮助的老师，说是导师也不为过。他没有在项目初期给我任何的建议，但在接下来的路径规划中起到了非常重要的指导作用。再次非常感谢他！不过未经过他的同意，我无法在这里提及他的任何信息，希望有耐心读到这里的每一位网友能够理解我的小小苦衷

回到关于接下来的计划。在完成当前阶段的重构之后，我逐渐意识到一个事实：如果继续停留在“调用现成接口”的层面，我只能是一个很不熟练的后端开发者（甚至不到及格线的后端新手），我之后的发展不够精通；哪怕说我开始尝试自己去微调或者训练出一个简单的模型，去“炼丹”，我最终也只是和其他所有人一样，走在了大部分人的同一条竞争道路之中。当然我不是说这不好，我只是觉得我没有足够的信心去竞争，也许另辟蹊径是一种比较好的选择？

我的导师在讲座中提到，企业更需要的是具有`独立将项目落地的经验`的人，还有`精通模型运维与生命周期管理`的人（我不知道这个讲的对不对，因为我真的懂得不多，表述可能确实不够规范）我并不是想做科研，也不是想在参数规模或 benchmark 上和任何人竞争，我只是了结目前最深的执念。在当前硬件条件（4060 Ti，8GB 显存）的限制下，我需要去做到：

> **一个大模型系统，在真实工程条件下被部署、监控、迭代和维护。**

因此，似乎接下来这一阶段的目标非常明确：
不追求我的项目最终能有多好，只追求 **可落地、可监控、可复现**。

我自己设想的整个计划（参考AI建议后+AI概括 得到的精确概括版）：

```plan
本项目围绕 AI 伴侣系统构建完整 MLOps 工程链路，基于 4060 Ti GPU，以 6-8 周实现从数据到部署的闭环：前 2 周完成微信聊天记录脱敏清洗与标准化数据集构建，部署 4-bit 量化 7B 模型（如 Llama-3-8B-Instruct），通过 FastAPI 接入现有服务并搭建基础日志与性能基线；第 3-4 周接入 MLflow，定义吞吐量、p95 延迟、推理成本项目将围绕 AI 伴侣系统，逐步引入大语言模型与 MLOps 工程实践，目标是在可控算力与有限时间内，完成一次从数据准备、模型推理、微调到部署与监控的完整工程闭环。整体周期预计 6–8 周，硬件基线为 RTX 4060 Ti 单卡环境，重点不在追求模型规模或 SOTA 指标，而在于真实可落地、可复现、可反思的工程实践。

在 第 1–2 周，工作将聚焦于数据层建设：对微信聊天记录进行脱敏、清洗与结构化处理，明确数据边界与风格目标，构建可用于监督微调的标准化数据集。同时完成基础推理环境的搭建，部署 4-bit 量化的 7B 级指令模型（如 Llama-3-8B-Instruct），并通过 FastAPI 接入现有服务体系，建立最小可用的模型推理链路，同时记录初始吞吐量、延迟等性能基线。

在 第 3–4 周，引入 MLflow 作为实验与运行记录工具，对模型推理过程进行系统化观测。重点关注吞吐量、p95 延迟、推理成功率等核心指标，在连续多日运行中暴露潜在的工程问题（如显存波动、请求堆积、异常恢复能力不足等），以“系统是否稳定”而非“模型是否聪明”作为主要评判标准。

在 第 5–6 周，基于已验证稳定的推理链路，使用 PEFT 框架进行 SFT + LoRA 微调实验，使模型在保持通用能力的前提下逐步贴合目标交互风格。通过 MLflow 对比不同数据规模、LoRA 参数与训练策略下的效果变化，并尝试引入 vLLM 优化推理性能，在不显著增加硬件成本的前提下提升响应效率，最终沉淀一份可复用的工程经验总结。

在条件允许的情况下，第 7–8 周可作为扩展探索阶段，尝试 ONNX 模型转换与部署，对比不同推理后端在兼容性、性能与维护成本上的差异，从工程视角评估其实际价值，而非单纯追求技术“完成度”。

整个过程将始终围绕 “可落地、可监控、可复现” 三个原则展开，不以炫技为目的，而是希望通过一次完整而克制的实践，让模型真正成为系统的一部分，而非孤立存在的实验品。功率等核心指标，经多日稳定运行排查系统隐患；第 5-6 周基于 PEFT 库采用SFT+LoRA方式微调以贴合目标风格，用 MLflow 对比实验效果，接入 vLLM 优化推理速度并输出工程总结；可选第 7-8 周开展 ONNX 模型转换与部署，从兼容性、性能等维度做工程价值分析，全程聚焦可落地、可监控、可复现，沉淀完整工程实践成果。
```

哇偶，好厉害，原来我自己磕磕绊绊口述出来的内容，最终居然这么高大上

这一整套计划，并不是为了证明我“会大模型”，也不是为了让他看到“没想到吧其实我可是比你厉害的人啊”，而是为了让我自己清楚我现在在哪，我在解决什么问题，和我为什么要这样设计

也许这条路走得很慢，和真正厉害的人差距依然很大，和他之间的差距也很大，甚至有非常多的人早就开始了这些工程实践

是的，我永远也追不上那些天赋异禀的人，追不上有着清晰职业规划与未来规划的人，甚至永远也追不上我喜欢的人的那个水平

但至少，我不再是原本那个只懂得打游戏 课程过了就行的人，我不再是想着毕业后成为“三无学生”靠蒙骗混进公司的人，我再不是被人看不起、被人拒绝、没有任何选择权只能和没有任何能力的人组队的那种人

也许我真的慢慢变得和以前的我不一样了

# 关于解决本地小模型知识储备不足的四种技术路径设想

在我整理话题列表，特别是技术类话题时，我发现像"RESTful API设计原则"、"Docker容器化部署"等专业问题，这些很多都是非常真实的技术问题，需要对应的回答问题的能力。本地部署的模型 可能因为预训练数据的局限性，无法给出准确或可执行的命令------甚至说能不能识别 需要输出具体命令 这样的情景都是一个问题！因为它的预训练的样本可能并不包含这部分的内容！我想到了这个问题之后，设想了四条路径：

```
1.下载所有需要的指导文档之类的内容，作为知识库，去专门训练一个Expert LLM专家模型，然后在用户询问对应技术问题的时候，调用专家模型生成回复，算是初步实现MoE的调度路由与专家模型？不知道能不能算是一个MoE架构？

2.当询问对应的技术问题，得到对应的规范化输出，输出端的监听组件 检测到规范化输出（例如json格式的内容）之后，调用爬虫进行搜索对应信息，返回对应信息之后 调用原本的LLM进行总结并输出对应内容

3.当询问对应的技术问题，得到对应的规范化输出，输出端的监听组件 检测到规范化输出（例如json格式的内容）之后，调用AI服务厂商的API，得到高性能AI所输出的回复之后，直接把结果输出给用户

4.调用AI服务厂商的API，得到高性能AI所输出的回复，然后把这种回复喂给LLM进行微调
```

经过AI的整理之后，我的思路可以被归为下面的内容：

1.  **专家模型 (Expert LLM) / MoE 思路**
    *   **原理**：收集特定领域的指导文档、技术手册等构建知识库，专门训练一个垂直领域的"专家模型"。
    *   **流程**：当用户询问特定技术问题时，系统路由（Router）将请求转发给这个专家模型生成回复，而不是通用的聊天模型。
    *   **思考**：这有点像 Mixture of Experts (MoE) 的思想，只是这里的 Expert 是独立训练的特定模型。

2.  **RAG + 爬虫 (Search-based RAG)**
    *   **原理**：利用外部实时信息补充模型知识。
    *   **流程**：
        1.  本地模型识别用户意图，输出规范化的结构数据（如 JSON），指示需要搜索的关键词。
        2.  输出端组件捕获该指令，调用爬虫或搜索 API 获取相关技术文档或网页内容。
        3.  将搜索到的内容作为上下文（Context）喂回给本地模型（或一个专门的总结模型），由其进行总结并生成最终回答。

3.  **混合智能 / API 回退 (Hybrid Intelligence / API Fallback)**
    *   **原理**：遇到难题直接"场外求助"，利用高性能商用模型解决复杂问题。
    *   **流程**：
        1.  本地模型识别出这是高难度技术问题，输出规范化的结构数据（如 JSON）。
        2.  输出端组件捕获指令，直接调用 AI 服务厂商（如 DeepSeek, OpenAI）的高性能 API。
        3.  将云端大模型的回答直接透传给用户，确保准确性和专业度。

4.  **模型蒸馏 / 持续微调 (Distillation / Fine-tuning)**
    *   **原理**：利用大模型的输出来"教会"小模型。
    *   **流程**：
        1.  平时通过调用高性能 AI 厂商的 API 获取高质量回复。
        2.  收集这些 {问题, 高质量回答} 对作为训练数据。
        3.  定期使用这些数据对本地 LLM 进行微调（SFT），将大模型的知识"内化"到本地小模型中，逐步提升其自身的回答能力。

但是其实似乎已经有一种非常成熟的解决方案了，这也似乎是目前行业里“相对最优解”

我上述所说的方法，有一部分是希望模型更"聪明"甚至成为专家（或者分出子模型 让子模型成为专家），有一部分是把这个难题分给其他厂商去解决。前者仍旧无法避免可能存在的幻觉问题，而后者则是逃避这个问题，反正我个人能力有限 就在这里了

Cloude Code的Skills机制，可能是在强调：模型幻觉始终存在，让模型始终保持输出绝对可行的内容很难，但是模型学会“调用现实世界的能力”却很简单

类似Cloude Code那一步可能太远，但是实现简单的skill应该还是可以的

# 不仅仅是需要Skills，还需要有一个完善的安全架构，防止恶意输入的同时也要审核输出！

之前提到过我的指导老师，他曾经在讲座中提及过 数据集的投毒 还有Skewing攻击之类的问题，本质上都是在强调一个安全问题：**模型的输入输出是否符合合规**。

目前这个只是一个非常简单的项目，我只需要实现一个基本的安全架构：在LLM推理前判断请求是否安全，输出后判断内容是否可以直接返回，给 Router 一个明确、结构化的决策结果，并且最终所有判断结果 必须可被日志系统记录

思路好像慢慢开始变得很清晰了。总之先实现这种最基础的安全架构，后续再考虑更复杂的场景